...
//include { PRE_PROCESSING_WORKFLOW } from '../pre_processing_workflow/pre_processing_workflow.nf'
...
/*
    PRE_PROCESSING_WORKFLOW (
        samplesheet
    )
    samplesheet = PRE_PROCESSING_WORKFLOW.out.processed_samplesheet.collect()
    multiqc_files_ch = PRE_PROCESSING_WORKFLOW.out.multiqc_files.collect()
*/
...
*/
    ...
    genome_bai_ch = SAMTOOLS_SORT.out.bai
        .map { meta, bai ->
            return [ bai ] }
    bam_bai_ch = bam_sorted_ch.combine(genome_bai_ch)

if (!params.skip_umi) {

        UMITOOLS_DEDUP (
        bam_bai_ch,
        params.umi_output_stats
    )
    bam_sorted_ch = UMITOOLS_DEDUP.out.bam
    versions_ch = versions_ch.mix(UMITOOLS_DEDUP.out.versions)
